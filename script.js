...
(() => {
  ...
  (() => {
    ...
    const phrases = ['build AI & ship it.', 'ship an AI tutor to 10 courses.', 'co-found a 7,000-student network.', 'put voice agents in production.', 'trained models on a supercomputer.', 'build the OS for autonomy.'];
    let i = 0;
    setInterval(() => {
  ...
  (() => {
    ...
    dog.addEventListener('click', () => {
      ...
      t1 = setTimeout(() => dog.classList.remove('barking'), 980);
  ...
  (() => {
    ...
    const PROJECTS = [
      { meta: 'Best Project, SDTC (1 of 30 from 200+), SAIRS 2025', title: 'ORCA', body:
        "<h4>The problem</h4><p>Sheet music and audio live in two different worlds, and moving between them by hand is slow and error-prone. Most tools, where they exist at all, only go one direction.</p><h4>Where it came from</h4><p>Built inside the Innovate Research Group, the cross-disciplinary group Pranav founded. ORCA won Best Project at the San Diego Undergrad Tech Conference, picked as 1 of 30 from 200+ entries, and was shown at SAIRS 2025.</p><h4>How it works</h4><p>ORCA is deliberately two-way. OpenCV reads and segments the printed score, a stack of neural nets separates the individual instruments, and a CNN transcribes audio back into clean notation. The accuracy was held to 98% and verified four different ways, including a blind check by real musicians rather than a single benchmark. Stack: OpenCV, CNNs, Flask, Docker, React.</p>" },
      { meta: 'Triton Software Engineering, EM of 11, raised $75K', title: 'Spay.LA', body:
        "<h4>The problem</h4><p>Spay.LA, a nonprofit, needed a real fundraising and advocacy proposal site, not a template, and someone to carry it from a vague brief to something shipped.</p><h4>Where it came from</h4><p>Built through Triton Software Engineering, UC San Diego's pro-bono studio that has shipped software for 25+ nonprofits over eight years. Pranav led the build as engineering manager of a team of 11.</p><h4>What it took</h4><p>He owned the full arc: scoping with the client, running the team, and delivering. It raised $75K. The hard part was running a team and a client relationship at once, turning a fuzzy ask into a concrete build and keeping eleven people pointed the same direction long enough to ship. <a href=\"https://www.spay.la/\" target=\"_blank\" rel=\"noopener\">spay.la</a></p>" },
      { meta: 'Innovate Research Group', title: 'AIDE', body:
        "<h4>The problem</h4><p>Most AI coding tools generate plausible code and leave you to find out whether it runs. The gap is between something that looks right and something that actually works in your project.</p><h4>Where it came from</h4><p>One of Pranav's Innovate Research Group projects.</p><h4>How it works</h4><p>AIDE (AI Integrated Developer Environment) is an intent-based, zero-touch IDE: you describe what you want and it produces working code through multi-layer intent classification, slot-filling for the missing details, and direct code injection. The key decision is the runtime correctness loop: generated code is wired into CI/CD and checked against real execution context, so AIDE can catch and fix its own mistakes instead of handing you something broken.</p>" },
      { meta: 'Innovate Research Group', title: 'AgentSpace', body:
        "<h4>The problem</h4><p>An expert's way of working only scales as far as their own hours, and a chatbot that sounds like them is not the same as one that carries their skills and judgment.</p><h4>Where it came from</h4><p>One of Pranav's Innovate Research Group projects.</p><h4>How it works</h4><p>AgentSpace is a two-way marketplace: professionals pour their personality and toolset into a hireable agent, and users hire that agent and put it to work. It runs on two-way GraphRAG over both skill and personality embeddings (in the spirit of Gemini Gems and Claude Skills), and the agents are exposed over MCP and A2A so they can share context and delegate to each other. The result is less a single chatbot and more a network of specialists that collaborate.</p>" },
      { meta: 'Lab for Emerging Intelligence', title: 'BioVeritas', body:
        "<h4>The problem</h4><p>The volume of published research keeps growing, and a lot of it is weaker or more overstated than it looks. Reading a paper does not tell you how much to trust it.</p><h4>Where it came from</h4><p>Built in the Lab for Emerging Intelligence.</p><h4>How it works</h4><p>BioVeritas reads a piece of biology research and judges how sound and honest it is. Instead of summarizing, it interrogates: checking claims, flagging weak or overstated results, and showing where the evidence does and does not hold up. Every flag is backed by retrieval, pointing to the supporting or contradicting literature rather than asking you to trust it.</p><h4>The idea</h4><p>It is Pranav's take on pointing AI at scientific rigor instead of producing more unchecked text.</p>" },
      { meta: 'Triton Software Engineering, developer', title: 'Union Station Housing', body:
        "<h4>The problem</h4><p>Union Station Homeless Services, one of greater Los Angeles's largest providers of housing for people experiencing homelessness, needed a fast, reliable internal portal for managing listings at scale.</p><h4>Where it came from</h4><p>Another Triton Software Engineering build, UC San Diego's pro-bono studio for nonprofits. Pranav worked on it as a software developer.</p><h4>What it does</h4><p>Photo and video upload for each listing, validation on the forms staff fill out, and bulk data exports so the team can pull everything at once. Unglamorous but real: the kind of media handling and data tooling an internal team actually uses day to day.</p>" },
      { meta: 'Innovate Research Group', title: 'Skin Lesion Detector', body:
        "<h4>The problem</h4><p>Skin-lesion screening is most useful exactly where the hardware is weakest: no GPU, no reliable connection.</p><h4>Where it came from</h4><p>One of Pranav's Innovate Research Group projects.</p><h4>The key constraint</h4><p>An image classifier that spots skin lesions from a photo, built to run on small on-device hardware rather than a server. That constraint shaped everything: the model has to be accurate enough to be useful and light enough to run where there is no GPU. It is the recurring theme in his work, getting a capable model to run somewhere it can reach the person who needs it.</p>" },
      { meta: 'Innovate Research Group', title: 'Virtual Trial Room', body:
        "<h4>The problem</h4><p>Online shopping still makes you guess how something will look on you, and guesses come back as returns.</p><h4>Where it came from</h4><p>One of Pranav's Innovate Research Group projects.</p><h4>How it works</h4><p>A 3D virtual fitting room using deep learning and computer vision to show how clothes look without physically trying them on. It reconstructs the garment and the wearer in 3D so the fit and drape read as believable rather than a flat overlay.</p>" }
    ];
    const LEADS = [
      { meta: 'Co-founder and president', title: 'Real World AI Network (RAIN)', body:
        "<h4>What it is</h4><p>RAIN (Real World AI Network) calls itself the operating layer for real-world AI at UC San Diego. Pranav co-founded it to consolidate the university's scattered AI efforts, research labs, project teams, hackathon crews, and founder collectives, into one ecosystem spanning the engineering, data science, and business schools.</p><h4>The scale</h4><p>7,000+ students, 17+ federated labs and builder clubs, and 38 aligned faculty, organized around four pillars: student orgs, faculty, industry, and entrepreneurship.</p><h4>The role</h4><p>Co-founder and president. He runs the machine behind it: quarterly research and innovation cycles, the RAIN Fellowship, a founder pipeline that takes ideas to prototypes to startups, hackathons, and VC panels. At that scale the community around a problem matters as much as the problem. <a href=\"https://rain.ucsd.edu\" target=\"_blank\" rel=\"noopener\">rain.ucsd.edu</a></p>" },
  ...
  (() => {
    ...
    const KB = [
      ...
      { re: [/\b(lab for emerging intelligence|\blei\b|agentic (platform|system|ai)|workflow (engine|executor)|mcp server|enterprise agent|7,?000)\b/], kw: ['agentic', 'platform', 'mcp', 'workflow', 'enterprise'], a: "At the Lab for Emerging Intelligence (Teradata-funded) he leads an enterprise agentic-AI platform, in pilot with 100+ UCSD staff and faculty: custom MCP servers over the university's APIs, research into automated skill generation and optimization, and a workflow executor he built to stay intelligent, deterministic, and graceful when a task goes sideways." },
      { re: [/\b(tutor|teaching (bot|assistant)|smart learning|8,?000 student|panel.?of.?judges|eval(uation)? pipeline|knowledge tracing)\b/], kw: ['tutor', 'teach', 'student', 'learning', 'eval', 'education-tech'], a: "He built and shipped an AI tutor now running in 10 courses across UCSD, SDSU, and Cal Poly Pomona. To keep it reliable he built a two-layer evaluation pipeline: one layer monitors tutor outputs live, and an LLM panel-of-judges above it stores each student's learning and personality metrics so the teaching adapts. A controlled study over 600+ students: 64% fewer TA overtime hours and 87% higher engagement." },
      ...
      { w: 4, re: [/\b(research|researching|papers?|publication|thesis|academ(ic|ia)|stud(y|ies)|scholar)\b/], kw: ['research', 'paper', 'thesis', 'study', 'publication'], a: "Four research threads: computational redistricting (his CSE honors thesis, now his master's thesis too), university-wide agentic systems at LEI, the Smart Learning Hub plus AI tutor, and AILA/LifeSaver multimodal health AI. He likes problems where the model has to be both capable and trustworthy. The research section is interactive, go poke at any of them." },
      { re: [/\b(redistrict\w*|gerrymander\w*|congress\w*|district\w*|fairness|honors thesis|efficiency gap|voting rights|census|master'?s thesis|\bmaps?\b)\b/], kw: ['redistricting', 'gerrymandering', 'districts', 'thesis', 'census'], a: "His honors thesis draws and scores U.S. congressional maps over Census data (158K to 5.5M blocks per state). He built three map generators (a greedy heuristic, a Monte Carlo sampler, and an XGBoost model with graph partitioning) and wrote the fairness metrics that grade them, like the efficiency gap and Voting Rights Act compliance. It reproduces the real district counts for Alabama, California, and Texas. It started as his CSE honors thesis and is now his master's thesis too." },
      ...
      { re: [/\b(hir(e|ing)|(still )?looking( for)?|seeking|open to|availab\w*|opportunit\w*|roles?|positions?|recruit\w*|relocat\w*|locat(ion|ed)|based|bay area|san francisco|sf\b|start date|when can he start)\b/], kw: ['hire', 'looking', 'available', 'relocate', 'location', 'role'], a: "Yes, he's looking, for full-time roles. He finishes his CS master's at UC San Diego in December 2026. In September he also picks up a part-time engagement as founding software engineer at a stealth startup building autonomy software, a few hours a week alongside the degree, so it doesn't take him off the market. He's after software, ML, or forward-deployed engineering at top AI companies, ideally where the models are biggest and the stakes are real. He's based in San Diego and happy to relocate anywhere in the SF Bay Area. Email is fastest for specifics: <a href=\"mailto:prsoma@ucsd.edu\">prsoma@ucsd.edu</a>." },
